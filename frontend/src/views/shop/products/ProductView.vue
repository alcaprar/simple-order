...
import { UnitType, fromString, toString } from '../../../models/unit_type'
import type { ProductDto } from '@/api_contracts/product'
export default {
  data() {
    return {
      product: {
  ...
  methods: {
    ...
    async getProduct(productId: string): Promise<Product> {
      ...
      let product_response: ProductDto = (await response.json()).data.attributes
      this.$log.debug('getProduct', product_response)

      return {
        id: product_response.id,
        name: product_response.name,
        unit: fromString(product_response.unit)
      }
    },
    async onSave() {
      if (this.isNew()) {
        const productId = await this.createProduct(this.product.name, this.product.unit)
        ...
        const productId = this.$route.params.productId as string
        await this.updateProduct(productId, this.product.name, this.product.unit)
      }
    },
